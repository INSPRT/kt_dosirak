    ...
    <div class="wrap">
           <div class="wrapAuto">
                ...
                <div class="item_listWrap">
                    <div class="item_list">
                        <ul>
                        <c:forEach var="productDto" items="${chestList}">
                            <li>
                                <div class="item_list_img">
                                    <a href="<c:url value="/chest/detail?product_id=${productDto.product_id}"/>">${productDto.product_desc}</a>
                                </div>
                                <div class="item_list_txt">
                                    <a href="<c:url value="/chest/detail?product_id=${productDto.product_id}"/>">${productDto.product_name}</a><br>가격: <span id="indi_price${productDto.product_id}">${productDto.product_price}</span> 원
                                </div>
                            </li>
                        </c:forEach>
                        </ul>
                    ...
                    <div class="pageHandle">
                        ...
                        <c:if test="${ph.totalCnt!=null && ph.totalCnt!=0}">
                            <c:if test="${ph.showPrev}">
                                <a class="page" href="<c:url value="/chest/itemlist?page=${ph.beginPage-1}"/>">&lt;</a>
                            </c:if>
                            <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
                                <a class="page ${i==ph.page? "paging-active" : ""}" href="<c:url value="/chest/itemlist?page=${i}"/>">${i}</a>
                            </c:forEach>
                            <c:if test="${ph.showNext}">
                                <a class="page" href="<c:url value="/chest/itemlist?page=${ph.endPage+1}"/>">&gt;</a>
                            </c:if>
                        </c:if>
                    </div>
                </div>
