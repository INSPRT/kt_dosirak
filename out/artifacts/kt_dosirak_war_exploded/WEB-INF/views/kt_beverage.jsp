           ...
           <div class="wrapAuto">
                ...
                <div class="itemListHeader">
                    <ul class="itemListHeader_first">
                        ...
                        <li class="itemListHeader_first_title">음료 ${mode=='admin' ? "| <a href='/chest/write'> 등록</a>" : ''}</li>
               ...
               <div class="item_listWrap">
                   <div class="item_list">
                       <ul>
                           <c:forEach var="productDto" items="${beverageList}">
                               <li>
                                   <div class="item_list_img">
                                       <a href="<c:url value="/beverage/detail?product_id=${productDto.product_id}"/>">${productDto.product_desc}</a>
                                   </div>
                                   <div class="item_list_txt">
                                       <a href="<c:url value="/beverage/detail?product_id=${productDto.product_id}"/>">${productDto.product_name}</a><br>가격: <span id="indi_price${productDto.product_id}">${productDto.product_price}</span>
                                   </div>
                               </li>
                           </c:forEach>
                       </ul>
                   ...
                   <div class="pageHandle">
                       ...
                       <c:if test="${ph.totalCnt!=null && ph.totalCnt!=0}">
                           <c:if test="${ph.showPrev}">
                               <a class="page" href="<c:url value="/beverage/itemlist?page=${ph.beginPage-1}"/>">&lt;</a>
                           </c:if>
                           <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
                               <a class="page ${i==ph.page? "paging-active" : ""}" href="<c:url value="/beverage/itemlist?page=${i}"/>">${i}</a>
                           </c:forEach>
                           <c:if test="${ph.showNext}">
                               <a class="page" href="<c:url value="/beverage/itemlist?page=${ph.endPage+1}"/>">&gt;</a>
                           </c:if>
                       </c:if>
                   </div>
               </div>
